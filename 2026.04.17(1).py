"""
@file main.py
@brief Непрерывный прорыв: Бесконечный рогалик - главный модуль игры
@details Содержит инициализацию игры, загрузку ресурсов, систему эффектов и другие основные функции
"""
...
pygame.display.set_caption("Борьба с невзгодами")
...
def get_res_path(path):
    """
    @brief Получение абсолютного пути к файлу ресурса
...
def load_frames(image_paths, fallback_color, size, shape="rect"):
    """
    @brief Загрузка последовательности кадров из изображений
    @param image_paths Список путей к изображениям
    @param fallback_color Цвет запасного варианта 
    @param size Размер кадра 
    @param shape Форма запасного варианта 
    @return list Список поверхностей с кадрами
    """
    frames = []
...
class DamageText(pygame.sprite.Sprite):
    """
    @brief Класс всплывающего текста урона/лечения
    """
    def __init__(self, x, y, damage_val, is_heal=False, custom_color=None, is_text=False):
        """
        @param x Координата X 
        @param y Координата Y 
        @param damage_val Значение урона или лечения
        @param is_heal True если это лечение 
        @param custom_color Пользовательский цвет 
        @param is_text True если отображать как обычный текст без знаков +/-
        """
        super().__init__()
        self.x, self.y = x, y
    ...
    def update(self):
        """Обновление состояния: подъём вверх и таймер"""
        self.timer += 1; self.y -= 1
        if self.timer >= self.lifetime: self.kill()

    def draw(self, surface, camera_x, camera_y):
        """Отрисовка текста с эффектом затухания"""
...
class ExplosionEffect(pygame.sprite.Sprite):
    ...
    def update(self):
        """Обновление таймера, уничтожение по окончании"""
        self.timer += 1
...
class LaserBeamEffect(pygame.sprite.Sprite):
    """
    @brief Класс эффекта лазерного луча
    """
    def __init__(self, x, y, angle, length, is_skill):
        """
        @param x Координата X источника луча 
        @param y Координата Y источника луча 
        @param angle Угол направления луча 
        @param length Длина луча
        @param is_skill True если это навык 
        """
        super().__init__()
        self.x, self.y = x, y
    ...
    def draw(self, surface, camera_x, camera_y):
        """Отрисовка лазерного луча с эффектом затухания"""
        alpha = int(255 * (1 - self.timer / self.lifetime))
...
class MeleeSwingEffect(pygame.sprite.Sprite):
    """
    @brief Класс эффекта взмаха оружия 
    """
    def __init__(self, start_x, start_y, attack_range, angle, weapon_img, is_skill_active, boss_override_color=None):
        """
        @param start_x Координата X начала атаки 
        @param start_y Координата Y начала атаки 
        @param attack_range Радиус атаки
        @param angle Угол направления атаки 
        @param weapon_img Изображение оружия
        @param is_skill_active True если это навык 
        @param boss_override_color Цвет босса 
        """
        super().__init__()
    ...
    def draw(self, surface, camera_x, camera_y):
        """Отрисовка секторного взмаха с оружием"""
...
class FlameEffect(pygame.sprite.Sprite):
    """
    @brief Класс эффекта пламени (конусообразная атака)
    """
    def __init__(self, start_x, start_y, attack_range, angle, is_skill=False):
        """
        @param start_x Координата X источника пламени (в мире)
        @param start_y Координата Y источника пламени (в мире)
        @param attack_range Дальность атаки
        @param angle Угол направления (в радианах)
        @param is_skill True если это навык (фиолетовый цвет)
        """
        super().__init__()
        self.x, self.y, self.range = start_x, start_y, attack_range
        self.lifetime, self.timer = 15, 0 
        ...
        # 喷射火焰特效
        self.frames = load_frames(["flame1.png", "flame2.png", "flame3.png", "flame4.png"], 
                                  (255, 100, 100, 0), (self.range*2, self.range*2), "rect")
    ...
    def draw(self, surface, camera_x, camera_y):
        """Отрисовка конусообразного пламени с анимацией"""
        s = pygame.Surface((self.range*2, self.range*2), pygame.SRCALPHA)
        points = [(self.range, self.range)]
...
class BossAoeEffect(pygame.sprite.Sprite):
    """
    @brief Класс AoE-эффекта босса 
    """
    def __init__(self, x, y, radius, damage, player):
        """
        @param x Координата X центра зоны (в мире)
        @param y Координата Y центра зоны (в мире)
        @param radius Радиус зоны поражения
        @param damage Урон за тик
        @param player Ссылка на игрока (для проверки попадания)
        """
        super().__init__()
        self.x, self.y, self.radius, self.damage, self.player = x, y, radius, damage, player
        self.warning_time = 60  # 激活前的时间
        self.lifetime = 1260   # 总寿命
        self.timer = 0
    ...
    def update(self):
        """Обновление: предупреждение → активация урона → уничтожение"""
        self.timer += 1
    ...
    def draw(self, surface, camera_x, camera_y):
        """Отрисовка зоны: предупреждение (красный круг) → активная зона"""
...
class BossSwordAuraBullet(pygame.sprite.Sprite):
    """
    @brief Класс-снаряда босса 
    """
    def __init__(self, x, y, angle, damage, speed, range_scale):
        """
        @param x Координата X спавна (в мире)
        @param y Координата Y спавна (в мире)
        @param angle Угол полёта (в радианах)
        @param damage Урон снаряда
        @param speed Скорость полёта
        @param range_scale Масштаб снаряда
        """
        super().__init__()
        self.b_type = 3
    ...
    def update(self):
        """Движение снаряда, проверка столкновения со стенами и таймер"""
        self.rect.x += self.vx
        self.rect.y += self.vy
        self.timer -= 1
...
class Sandbag(pygame.sprite.Sprite):
    """
    @brief Класс манекена для тестирования урона (DPS-мишень)
    """
    ...
    def take_damage(self, amount, effects_group=None, **kwargs): 
        """
        @brief Получение урона манекеном
        @param amount Количество урона
        @param effects_group Группа спрайтов для отображения текста урона
        """
        self.hit_timer = 8
    ...
    def update(self):
        """Обновление: возврат в состояние покоя после анимации попадания"""
        if self.hit_timer > 0:
            self.hit_timer -= 1
            if self.hit_timer <= 0:
                self.image = self.idle_img
    ...
    def draw_dps(self, surface, camera_x, camera_y):
        """
        @brief Отрисовка панели DPS (урон в секунду)
        @param surface Поверхность для отрисовки
        @param camera_x Смещение камеры по X
        @param camera_y Смещение камеры по Y
        """
        if self.last_hit_time > 0 and pygame.time.get_ticks() - self.last_hit_time <= 2000:
            ...
            txt1 = font_base.render(f"время: {time_active:.1f}s", True, WHITE)
            txt2 = font_base.render(f"Общее количество травм: {int(self.total_damage)}", True, YELLOW)
            txt3 = font_base.render(f"DPS: {int(dps)}", True, RED)
...
class ThrownGrenade(pygame.sprite.Sprite):
    """
    @brief Класс брошенной гранаты (летит по траектории, взрывается при попадании)
    """
    def __init__(self, x, y, tx, ty, damage, base_damage, player, enemies, effects, crates=None, items=None, w_imgs=None, coins=None):
        """
        @param x Координата X старта 
        @param y Координата Y старта 
        @param tx Координата X цели 
        @param ty Координата Y цели 
        @param damage Итоговый урон
        @param base_damage Базовый урон 
        @param player Ссылка на игрока
        @param enemies Группа врагов
        @param effects Группа эффектов
        @param crates Группа ящиков 
        @param items Группа предметов 
        @param w_imgs Группа изображений оружия 
        @param coins Группа монет 
        """
        super().__init__()
        # 手榴弹
        ...
        self.vz = 8.0 # 初始垂直速度
        self.gravity = (2.0 * self.vz) / self.flight_ticks #重力
        self.is_exploded = False

    def update(self):
        """Обновление: полёт по дуге, анимация, проверка на взрыв"""
    ...
    def explode(self):
        """Взрыв: нанесение урона врагам и ящикам в радиусе, создание эффекта взрыва"""
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y):
        ...
        self.weapons = [
            {"type": "pistol", "name": "обычный пистолет", "damage": 25, "cd": 300},
            {"type": "melee", "name": "нож для ближнего боя", "damage": 30, "range": 80, "cd": 400}
        ]
    ...
    def update(self, is_dead=False, dx=0, dy=0):
        ...
        @param is_dead True если игрок мёртв
        @param dx Смещение по X 
        @param dy Смещение по Y 
        """
        if is_dead: 
            self.image = self.death_frames[0]; return
    ...
    def take_damage(self, damage, custom_effects_ref=None):
        """
        @brief Получение урона игроком
        @param damage Количество урона
        @param custom_effects_ref Группа эффектов (для отображения текста)
        @return True если игрок умер, False если выжил
        """
        if getattr(self, "orbit_shield_active", False):
            self.orbit_shield_active = False
            self.orbit_shield_timer = 0
            if custom_effects_ref is not None:
                custom_effects_ref.add
                (DamageText
                 (self.rect.centerx, self.rect.top - 20, "Сопротивление отбрасыванию", custom_color=(100,200,255), is_text=True))
            return False
    ...
    def process_attack(self, mouse_held, mx, my, camera_x, camera_y, bullets_group, enemy_bullets_group, effects_group, global_weapon_images, enemies_group, grenades_group, crates_group=None, items_group=None, coins_group=None):
        """
        @brief Обработка атаки игрока
        @param mouse_held Зажата ли левая кнопка мыши
        @param mx Координата X мыши 
        @param my Координата Y мыши 
        @param camera_x Смещение камеры по X
        @param camera_y Смещение камеры по Y
        @param bullets_group Группа пуль игрока
        @param enemy_bullets_group Группа пуль врагов
        @param effects_group Группа эффектов
        @param global_weapon_images Словарь с изображениями оружия
        @param enemies_group Группа врагов
        @param grenades_group Группа гранат
        @param crates_group Группа ящиков 
        @param items_group Группа предметов 
        @param coins_group Группа монет 
        @return list Список атак
        """
        if not self.weapons:
            return []
        ...
        if self.stolen_weap_idx == self.current_weapon:
            ...
            if (mouse_held or mouse_held_right) and pygame.time.get_ticks() % 60 == 0:
                effects_group.add(DamageText(self.rect.centerx, self.rect.top - 25, "Это оружие обезврежено", custom_color=(255,50,50), is_text=True))
            return []
    ...
    def draw_weapon(self, surface, camera_x, camera_y, mx, my, global_weapon_images):
        """
        @brief Отрисовка оружия в руке игрока
        @param surface Поверхность для отрисовки
        @param camera_x Смещение камеры по X
        @param camera_y Смещение камеры по Y
        @param mx Координата X мыши (на экране)
        @param my Координата Y мыши (на экране)
        @param global_weapon_images Словарь с изображениями оружия
        """
...
class MagicArrow(pygame.sprite.Sprite):
    """
    @brief Класс магической стрелы 
    """
    def __init__(self, x, y, angle, damage, player, enemies_group, is_skill=False):
        """
        @param x Координата X спавна 
        @param y Координата Y спавна 
        @param angle Угол полёта 
        @param damage Урон
        @param player Ссылка на игрока
        @param enemies_group Группа врагов 
        @param is_skill True если это навык 
    ...
    def update(self):
        """
        @brief Обновление: самонаведение на ближайшего врага, движение, отскок от стен
        """
...
class GroundItem(pygame.sprite.Sprite):
    """
    @brief Класс предмета на земле (оружие или зелье)
    """
    def __init__(self, x, y, item_type, weapon_data=None, weapon_img=None):
        """
        @param x Координата X спавна (в мире)
        @param y Координата Y спавна (в мире)
        @param item_type Тип предмета ("potion" или "weapon")
        @param weapon_data Данные оружия (словарь)
        @param weapon_img Изображение оружия
        """
        super().__init__()
    ...
    def draw_prompt(self, surface, camera_x, camera_y):
        """
        @brief Отрисовка подсказки о предмете
        @param surface Поверхность для отрисовки
        @param camera_x Смещение камеры по X
        @param camera_y Смещение камеры по Y
        """
        txt = f"Нажмите [F] чтобы подобрать: {self.weapon_data['name']}" if self.item_type == "weapon" else "Нажмите [F], чтобы взять предмет [Восстановить запасы]."
...
class WeaponStand(pygame.sprite.Sprite):
    """
    @brief Класс стенда с оружием (магазин специального оружия)
    """
    def __init__(self, x, y, w_data, w_img):
        """
        @param x Координата X спавна 
        @param y Координата Y спавна 
        @param w_data Данные оружия 
        @param w_img Изображение оружия
        """
        super().__init__()
        # 🖼️特殊武器商店
    ...
    def draw_prompt(self, surface, camera_x, camera_y):
        """
        @brief Отрисовка подсказки с ценой
        @param surface Поверхность для отрисовки
        @param camera_x Смещение камеры по X
        @param camera_y Смещение камеры по Y
        """
        txt = f"Нажмите [F] Купить | $ {self.price} | {self.w_data['name']}"
    ...
    def draw(self, surface, camera_x, camera_y):
        """
        @brief Отрисовка стенда с парящим оружием
        @param surface Поверхность для отрисовки
        @param camera_x Смещение камеры по X
        @param camera_y Смещение камеры по Y
        """
...
class Crate(pygame.sprite.Sprite):
    ...
    def take_damage(self, amount, items_group, effects_group, global_weapon_images, coins_group=None):
        """
        @brief Получение урона ящиком (при разрушении выпадают предметы)
        @param amount Урон
        @param items_group Группа предметов (для спавна лута)
        @param effects_group Группа эффектов
        @param global_weapon_images Словарь с изображениями оружия
        @param coins_group Группа монет
        """
...
class SpecialCrate(Crate):
    """
    @brief Класс специального ящика 
    @details Наследуется от Crate, имеет больше HP и гарантированный выпад оружия
    """
    def __init__(self, x, y):
        """
        @param x Координата X спавна 
    ...
    def take_damage(self, amount, items_group, effects_group, global_weapon_images, coins_group=None):
        """
        @brief Получение урона - при разрушении выпадает случайное оружие
        @param amount Урон
        @param items_group Группа предметов 
        @param effects_group Группа эффектов
        @param global_weapon_images Словарь с изображениями оружия
        ...
        if self.hp <= 0:
            ...
            w_type = random.choice([
                {"type": "pistol", "name": "Мощный пистолет", "damage": 35, "cd": 250},
                {"type": "melee", "name": "нож для ближнего боя", "damage": 30, "range": 80, "cd": 400},
                {"type": "melee", "name": "широкий меч", "damage": 70, "range": 140, "cd": 800},
                {"type": "pistol", "name": "лазерный пистолет", "damage": 15, "cd": 100},
                {"type": "flamethrower", "name": "Огнемёт", "damage": 15, "range": 150, "cd": 600},
                {"type": "bow", "name": "Волшебный лук", "damage": 65, "cd": 150},
                {"type": "grenade", "name": "ручная граната", "damage": 70, "cd": 600} 
            ])
...
class Enemy(pygame.sprite.Sprite):
    """
    @brief Базовый класс врага (ближний бой)
    """
    ...
    def take_damage(self, amount, is_true_damage=False, **kwargs):
        """
        @brief Получение урона врагом
        @param amount Количество урона
        @param is_true_damage True если игнорировать щит
        """
    ...
    def update(self, px, py, crates_group=None, enemies_group=None, effects_group=None, player=None, **kwargs):
        """
        @brief Обновление: движение к игроку с обходом препятствий
    ...
    def explode(self, enemies_group, player, effects_group):
        """Взрыв врага (при наличии навыка у игрока)"""
...
class RangedEnemy(Enemy):
    ...
    def update(self, px, py, enemy_bullets_group=None, crates_group=None, enemies_group=None, effects_group=None, player=None, **kwargs):
        """
        @brief Обновление: стрельба при приближении, иначе движение
        """
...
class HealerEnemy(Enemy):
    """
    @brief Класс врага-лекаря (лечит союзников, убегает от игрока)
    """
    def __init__(self, x, y, hp, floor):
        """
        @param x Координата X спавна 
        ...
        self.image = self.frames[0]
        self.heal_cd = 120 # 疗愈充电
        self.casting_timer = 0
        
    def update(self, px, py, crates_group=None, enemies_group=None, effects_group=None, player=None, **kwargs):
        """
        @brief Обновление: лечение союзников, отступление от игрока
        """
...
class Boss(Enemy):
    """
    @brief Класс босса (сложный враг с навыками и фазами)
    """
    def __init__(self, x, y, hp, floor, is_tutorial=False):
        ...
        @param hp Базовое здоровье
        @param floor Текущий этаж
        @param is_tutorial True если это учебный босс
        """
    ...
    def update(self, px, py, enemy_bullets_group=None, effects_group=None, player=None, crates_group=None, enemies_group=None, **kwargs):
        """
        @brief Обновление босса: управление щитом, навыками, анимациями
        """
        if self.shield <= 0:
        ...
        if self.skill_timer >= self.skill_cd:
            ...
            # 显示技能警告
            if self.casting_skill == "dash": self.skill_text = "[ВНИМАНИЕ] Блокировка спринта"
            elif self.casting_skill == "aoe": self.skill_text = "[ВНИМАНИЕ] Смертельно опасный набор ядов"
            elif self.casting_skill == "boss_heal": self.skill_text = "[ВНИМАНИЕ] Жизнь восстановлена"
            elif self.casting_skill == "vision": self.skill_text = "[ВНИМАНИЕ] Потеря зрения"
            elif self.casting_skill == "silence": self.skill_text = "[Предупреждение] Тишина"
            elif self.casting_skill == "summon": self.skill_text = "[ВНИМАНИЕ] Призыв миньонов"

    def draw_hp(self, surface, camera_x, camera_y):
        """Отрисовка полоски здоровья босса (в центре экрана)"""
        ...
        txt = font_large.render(f" Здоровье противника ：{int(max(0, self.hp))}  Щит: {int(max(0, self.shield))}", True, WHITE)
...
class EnemyStage2(Enemy):
    """
    @brief Враг второго этажа - солдат с мечом 
    """
    def __init__(self, x, y, hp, floor):
        """
        @param x Координата X спавна 
    ...
    def update(self, px, py, crates_group=None, enemies_group=None, effects_group=None, player=None, **kwargs):
        """
        @brief Движение к игроку и ближняя атака при приближении
        """
        dx, dy = px - self.rect.centerx, py - self.rect.centery
        dist = math.hypot(dx, dy)
...
class ShieldEnemyStage2(Enemy):
    """
    @brief Враг второго этажа - щитоносец (защищается щитом от пуль)
    """
    def __init__(self, x, y, hp, floor):
        """
        @param x Координата X спавна 
    ...
    def update(self, px, py, crates_group=None, enemies_group=None, effects_group=None, player=None, **kwargs):
        """Обновление: движение к игроку + поворот щита в сторону игрока"""
        super().update(px, py, crates_group=crates_group, enemies_group=enemies_group, effects_group=effects_group, player=player)
        #盾牌始终指向玩家
        self.facing_angle = math.atan2(py - self.rect.centery, px - self.rect.centerx)

    def draw_hp(self, surface, camera_x, camera_y):
        """Отрисовка здоровья и щита поверх врага"""
...
class HealerEnemyStage2(HealerEnemy):
    """
    @brief Враг-лекарь второго этажа (наследуется от HealerEnemy)
    """
    def __init__(self, x, y, hp, floor):
        """
        @param x Координата X спавна 
...
class BossStage2(Boss):
    """
    @brief Босс второго этажа (с уникальными навыками)
    """
    def __init__(self, x, y, hp, floor, is_tutorial=False):
        """
        @param x Координата X спавна 
    ...
    def update(self, px, py, enemy_bullets_group=None, effects_group=None, player=None, crates_group=None, enemies_group=None, **kwargs):
        """
        @brief Обновление босса второго этажа
        @param px Координата X игрока 
        @param py Координата Y игрока 
        @param enemy_bullets_group Группа пуль врагов
        @param effects_group Группа эффектов
        ...
        if self.skill_timer >= self.skill_cd:
            ...
            if pick == "heavy_slash":
                ...
                self.skill_text = "【Широкий радиус поражения】"
                ...
                self.skill_text = "【внезапный приступ】"

            # 👇 新护盾
            elif pick == "invincible_shield":
                self.invincible_shield_timer = 15 * 60   # 持续十五秒
                self.skill_text = "【Укрепленная крепость】"
                ...
                self.skill_text = "【Разоружение】"
                if effects_group is not None: effects_group.add(DamageText(player.rect.centerx, player.rect.top-40, "Конфискованное запрещенное оружие",
                                                                            custom_color=(255, 0, 0), is_text=True))
                
            elif pick == "sword_aura_buff":
                self.has_aura_buff = 600 
                self.skill_text = "【Включает в себя удар мечом с энергией】"
    ...
    def draw_hp(self, surface, camera_x, camera_y):
        """Отрисовка полоски здоровья (использует родительский метод)"""
        super().draw_hp(surface, camera_x, camera_y)
...
class EnemyStage3(Enemy):
    """
    @brief Враг третьего этажа (ближний бой)
    """
    def __init__(self, x, y, hp, floor):
        """
        @param x Координата X спавна 
...
class RangedEnemyStage3(RangedEnemy):
    """
    @brief Враг-стрелок третьего этажа
    """
    def __init__(self, x, y, hp, floor):
        """
        @param x Координата X спавна
...
class HealerEnemyStage3(HealerEnemy):
    """
    @brief Враг-лекарь третьего этажа
    """
    def __init__(self, x, y, hp, floor):
        """
        @param x Координата X спавна 
...
class EnemyBullet(pygame.sprite.Sprite):
    """
    @brief Класс пули врага (поддерживает разные типы и отскоки)
    """
    def __init__(self, x, y, angle, speed, damage, b_type=0, is_boss=False):
        ...
        @param damage Урон
        @param b_type Тип пули: 0-обычная, 1-отскакивающая, 2-зелёная
        @param is_boss True если пуля от босса 
        """
        super().__init__()
        self.b_type = b_type
    ...
    def update(self):
        """Движение пули с проверкой отскока от стен"""
        self.rect.x += self.vx
        if is_wall(self.rect.centerx, self.rect.centery):
...
class Coin(pygame.sprite.Sprite):
    """
    @brief Класс монеты (с анимацией вращения и магнитом)
    """
    def __init__(self, x, y):
        """
        @param x Координата X спавна 
    ...
    def update(self, player):
        """
        @brief Обновление: анимация вращения + притяжение к игроку (магнит)
        @param player Ссылка на игрока
        """
...
class Portal(pygame.sprite.Sprite):
    """
    @brief Класс портала (анимированный, для перехода на следующий этаж)
    """
    def __init__(self, x, y):
        """
        @param x Координата X спавна 
    ...
    def update(self):
        """Анимация вращения кадров портала"""
        self.current_frame += 0.2
        self.image = self.frames[int(self.current_frame) % len(self.frames)]
        
    def draw(self, surface, camera_x, camera_y):
        """
        @brief Отрисовка портала с учётом камеры
        @param surface Поверхность для отрисовки
        @param camera_x Смещение камеры по X
        @param camera_y Смещение камеры по Y
        """
...
class SpawnerWarning:
    """
    @brief Класс предупреждения о спавне врагов
    """
    def __init__(self, x, y, e_type="normal"):
        """
        @param x Координата X 
        @param y Координата Y 
        @param e_type Тип врага ("normal", "ranged", "healer")
        """
...
class Room:
    """
    @brief Класс комнаты на карте
    """
    def __init__(self, x, y, w, h, is_start=False):
        """
        @param x Координата X комнаты 
        @param y Координата Y комнаты 
        @param w Ширина комнаты 
        @param h Высота комнаты 
        @param is_start True если это стартовая комната
        """
    ...
    def is_player_inside(self, px, py):
        """
        @brief Проверка, находится ли игрок внутри комнаты
        @param px Координата X игрока 
        @param py Координата Y игрока 
        @return bool
        """
...
def generate_map(floor, tutorial=False):
    """
    @brief Генерация карты для текущего этажа
    @param floor Номер этажа
    @param tutorial True если это обучающий режим
    @return tuple (spawn_x, spawn_y) - координаты спавна игрока
    """
...
def generate_training_map():
    """
    @brief Генерация карты для тренировочного режима (открытая арена)
    @return tuple (spawn_x, spawn_y) - координаты спавна
    """
    global game_map, room_list, explored_map, tile_variant_map
    game_map = [[4 for _ in range(MAP_COLS)] for _ in range(MAP_ROWS)]
...
def finalize_walls():
    """
    @brief Завершение генерации стен: определяет границы стен вокруг проходимых клеток
    """
    global game_map
...
def toggle_room_gates(room, close=True):
    """
    @brief Открытие/закрытие ворот комнаты
    @param room Объект комнаты
    @param close True - закрыть, False - открыть
    """
    if close:
        room.gates_coords = []
...
def draw_button(screen, text, x, y, w, h, color, hover_color, mx, my):
    """
    @brief Отрисовка интерактивной кнопки
    ...
    @param mx Координата X мыши
    @param my Координата Y мыши
    @return bool True если мышь на кнопке
    """
...
def draw_hud(screen, player, skill_icon_img):
    """
    @brief Отрисовка HUD (полоски HP, щита, монеты, навык)
    @param screen Поверхность для отрисовки
    @param player Ссылка на игрока
    @param skill_icon_img Иконка навыка
    """
    ...
    screen.blit(font_base.render(f"Щит: {player.shield:.1f}/{player.max_shield}", True, WHITE), (30, 52))
    screen.blit(font_large.render(f"золото: {coins}", True, YELLOW), (20, 80))
    ...
    if player.skill_timer > 0:
        ...
        skill_text, color = f"[Пробел] Действует: {player.skill_timer//60}s", (100, 200, 255)
        ...
        skill_text, color = f"Охлаждение: {player.skill_cd//60}s", GRAY
    else:
        skill_text, color = "[Космос] Навык готов", GREEN
...
def draw_minimap(screen, player, mode, c_floor):
    """
    @brief Отрисовка мини-карты в правом верхнем углу
    @param screen Поверхность для отрисовки
    @param player Ссылка на игрока
    @param mode Режим игры ("CAMPAIGN" или другой)
    @param c_floor Текущий этаж (для прогресс-бара)
    """
    ...
    if mode == "CAMPAIGN":
        ...
        txt = font_base.render(f"расписание: {min(c_floor, 30)}/30", True, WHITE)
        screen.blit(txt, (start_x + map_w//2 - txt.get_width()//2, bar_y + 10 - txt.get_height()//2))

def spawn_coins(x, y, is_boss, floor, coins_group):
    """
    @brief Спавн монет в указанной позиции
    @param x Координата X 
    @param y Координата Y 
    @param is_boss True если это босс (больше монет)
    @param floor Текущий этаж (влияет на количество)
    @param coins_group Группа монет
    """
...
def clear_all_systems_for_new_game():
    """
    @brief Сброс всех глобальных состояний для новой игры
    """
...
def main():
    ...
    global_weapon_images = {
        "обычный пистолет": load_frames(["普通手枪.png"], GRAY, (120, 70), "rect")[0],
        "Мощный пистолет": load_frames(["heavy_pistol.png"], (100,100,100), (45, 16), "rect")[0],
        "нож для ближнего боя": load_frames(["近战小刀.png"], WHITE, (168, 80), "rect")[0],
        "широкий меч": load_frames(["大刀.png"], WHITE, (190,74), "rect")[0],
        "пулемет": load_frames(["machine_gun.png"], (80,80,80), (60, 20), "rect")[0],
        "Огнемёт": load_frames(["flamethrower.png"], (255,69,0), (55, 16), "rect")[0],
        "Волшебный лук": load_frames(["magic_bow.png"], (138, 43, 226), (40, 70), "rect")[0],
        "ручная граната": load_frames(["grenade_icon.png"], DARK_GREEN, (20, 20), "circle")[0]
    }

    game_state, game_mode, coins, total_coins = "MENU", "ENDLESS", 0, 0
    current_floor, difficulty_name, death_timer, acquired_talents, shop_page = 1, "обычный", 0, [], 0
    prev_state = "" 
    boss_alive = False
    teleport_timer = 0
    ...
    def init_shop_items():
        return [
            {"id": "shield_max", "name": "Увеличить лимит щита", "cost": 5, "level": 0, "max": 99, "cost_up": 15},
            {"id": "melee_range", "name": "Увеличьте веерообразный диапазон", "cost": 15, "level": 0, "max": 99, "cost_up": 20}, 
            {"id": "ranged_mult", "name": "Увеличить урон от дальнего боя", "cost": 10, "level": 0, "max": 99, "cost_up": 5},
            {"id": "melee_mult", "name": "Увеличить урон в ближнем бою", "cost": 15, "level": 0, "max": 99, "cost_up": 8},
            {"id": "fir", "name": "Увеличьте скорость атаки", "cost": 15, "level": 0, "max": 6, "cost_up": 10},
            {"id": "spd", "name": "Увеличьте скорость передвижения", "cost": 15, "level": 0, "max": 5, "cost_up": 10},
            {"id": "hp", "name": "Восстанавливает 1 единицу здоровья", "cost": 5, "level": 0, "max": 999, "cost_up": 0},
            {"id": "explosion", "name": "Увеличение взрывной силы", "cost": 15, "level": 0, "max": 99, "cost_up": 15},
            {"id": "exp_range", "name": "Увеличьте радиус поражения", "cost": 15, "level": 0, "max": 15, "cost_up": 15}
        ]
    all_talents = [
        {"id": "bounce", "name": "Надувная стена для прыжков", "desc": "Пуля может отскочить один раз при ударе о стену", "unique": True},
        {"id": "hp_up", "name": "Жизнь вспыхивает", "desc": "Увеличивает максимальное здоровье на 2 и восстанавливает его полностью", "unique": False}, 
        {"id": "magnet", "name": "Магнит для золотых монет", "desc": "Крупномасштабный автоматический аттракцион с золотыми монетами", "unique": True},
        {"id": "explosion", "name": "Взрыв противника", "desc": "Победа над врагом вызовет взрыв", "unique": True},
        {"id": "weapon_slot", "name": "слот для оружия +1", "desc": "Разблокируйте дополнительные слоты для оружия", "unique": True},
        {"id": "skill_up", "name": "Специализация навыков", "desc": "Продолжительность действия активного навыка удвоена", "unique": True},
        {"id": "revive", "name": "Возрождение Феникса", "desc": "Воскресение в полном здравии после смерти", "unique": True},
        {"id": "scatter_up", "name": "Усиление рассеяния", "desc": "Все виды дробового оружия получают две дополнительные баллистические траектории", "unique": True},
        {"id": "charge_fast", "name": "Зарядите аккумулятор для максимальной скорости", "desc": "Скорость перезарядки такого оружия, как луки и стрелы, удваивается", "unique": True},
        {"id": "large_explosion", "name": "Гений взрывчатых веществ", "desc": "Радиус всех взрывов удвоился", "unique": True},
        {"id": "orbit_shield", "name": "Звездный щит", "desc": "Этот навык создает щит, который полностью блокирует следующую атаку в течение 15 секунд", "unique": True}
    ]
    ...
    def reset_floor(player_instance=None):
        """
        @brief Сброс этажа: генерация карты, спавн игрока, ящиков и магазина
        @param player_instance Существующий игрок (или None для создания нового)
        @return tuple (player, bullets, enemy_bullets, effects, enemies, coins, ...)
        """
        ...
        if shop_room_idx != -1:
            ...
            shop_weapons = [
                {"type": "pistol", "name": "Мощный пистолет", "damage": 35, "cd": 250},
                {"type": "melee", "name": "широкий меч", "damage": 70, "range": 140, "cd": 800},
                {"type": "pistol", "name": "пулемет", "damage": 15, "cd": 100},
                {"type": "flamethrower", "name": "Огнемёт", "damage": 15, "range": 150, "cd": 600},
                {"type": "bow", "name": "Волшебный лук", "damage": 65, "cd": 150},
                {"type": "grenade", "name": "ручная граната", "damage": 70, "cd": 600} 
            ]
    ...
    def init_training():
        """
        @brief Инициализация тренировочного режима
        @return tuple (player, bullets, enemy_bullets, effects, enemies, coins, ...)
        """
        nonlocal sandbags
        ...
        p.weapons = [
            ...
            {"type": "pistol", "name": "Мощный пистолет", "damage": 35, "cd": 250},
            {"type": "melee", "name": "нож для ближнего боя", "damage": 30, "range": 80, "cd": 400},
            {"type": "melee", "name": "широкий меч", "damage": 70, "range": 140, "cd": 700},
            {"type": "pistol", "name": "пулемет", "damage": 15, "cd": 100},
            {"type": "flamethrower", "name": "Огнемёт", "damage": 15, "range": 150, "cd": 600}, 
            {"type": "bow", "name": "Волшебный лук", "damage": 65, "cd": 150},
            {"type": "grenade", "name": "ручная граната", "damage": 70, "cd": 600}
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if game_state == "MENU":
                    ...
                    if btn_easy: difficulty_mult.update({"hp":0.7, "dmg":0.7, "spd":0.8, "range_hp":0.7, "range_spd":0.8}); difficulty_name="Простой"; game_state="MENU"
                    elif btn_norm: difficulty_mult.update({"hp":1.0, "dmg":1, "spd":1.0, "range_hp":1.0, "range_spd":1.0}); difficulty_name="обычный"; game_state="MENU"
                    elif btn_hard: difficulty_mult.update({"hp":1.8, "dmg":2, "spd":1.3, "range_hp":1.5, "range_spd":1.2}); difficulty_name="сложный"; game_state="MENU"
                    ...
                    for is_hover, t in talent_btns:
                        if is_hover:
                            if t['id'] == 'bounce': player.has_bounce = True
                                ...
                                player.weapon_slots += 1; player.weapons.append({"type": "pistol", "name": "запасной пистолет", "damage": 25, "cd": 250})
                    ...
                    if event.key == pygame.K_TAB: 
                        ...
                        for st in pedestals:
                            if math.hypot(player.rect.centerx - st.rect.centerx, player.rect.centery - st.rect.centery) < 70:
                                if coins >= st.price:
                                    coins -= st.price
                                    effects.add(DamageText(player.rect.centerx, player.rect.top-10, "Покупка прошла успешно", custom_color=(0, 255, 0), is_text=True))
                                    ...
                                    effects.add(DamageText(player.rect.centerx, player.rect.top-10, "Недостаточно золотых монет", custom_color=(255, 68, 68), is_text=True))
                                interacted = True
                                break
        ...
        if game_state == "MENU":
            ...
            screen.blit(font_title.render("Борьба с невзгодами", True, YELLOW), (SCREEN_WIDTH//2 - 200, 150))
            play_hover = draw_button(screen, "Бесконечный режим", SCREEN_WIDTH//2-100, 260, 200, 50, (50,150,50), (100,200,100), mx, my)
            play_campaign_hover = draw_button(screen, "Режим испытаний", SCREEN_WIDTH//2-100, 330, 200, 50, (150,100,50), (200,150,100), mx, my)
            play_tutorial_hover = draw_button(screen, "Руководство для начинающих", SCREEN_WIDTH//2-100, 400, 200, 50, (50,150,150), (100,200,200), mx, my)
            train_hover = draw_button(screen, "тренировочный лагерь", SCREEN_WIDTH//2-100, 470, 200, 50, (150,50,150), (200,100,200), mx, my)
            diff_hover = draw_button(screen, "Выберите уровень сложности", SCREEN_WIDTH//2-100, 540, 200, 50, GRAY, (150,150,150), mx, my)
            intro_hover = draw_button(screen, "Инструкции к игре", SCREEN_WIDTH//2-100, 610, 200, 50, (50,100,150), (100,150,200), mx, my)
            quit_hover = draw_button(screen, "Выйти из игры", SCREEN_WIDTH//2-100, 680, 200, 50, (150,50,50), (200,100,100), mx, my)
            
            screen.blit(font_base.render(f"Текущая сложность: [{difficulty_name}]", True, GRAY), (SCREEN_WIDTH//2 - 180, 750))
            pygame.display.flip(); clock.tick(FPS); continue
            
        elif game_state == "DIFF":
            screen.fill((20,20,30)); screen.blit(font_title.render("Выберите уровень сложности：", True, WHITE), (SCREEN_WIDTH//2-380, 150))
            btn_easy = draw_button(screen, "Простой режим", SCREEN_WIDTH//2-180, 300, 360, 60, (50,150,50), (100,200,100), mx, my)
            btn_norm = draw_button(screen, "Обычный режим", SCREEN_WIDTH//2-180, 400, 360, 60, (150,100,50), (200,150,100), mx, my)
            btn_hard = draw_button(screen, "Сложный режим", SCREEN_WIDTH//2-180, 500, 360, 60, (150,50,50), (200,100,100), mx, my)
            pygame.display.flip(); clock.tick(FPS); continue
            
        elif game_state == "INTRO":
            screen.fill((20,20,30))
            for i, line in enumerate(["Инструкции по игровому процессу：", 
                                    "[W][A][S][D] Перемещение. Управление прицеливанием с помощью мыши.", 
                                    "Клавиша TAB открывает магазин для покупки баффов, клавиша F взаимодействует с предметами и оружием, а клавиши 1/2 переключают текущее удерживаемое оружие.", 
                                    "Используйте пробел для активации навыков, следите за временем перезарядки.",
                                    "Выбирайте таланты, покупайте усиления и комбинируйте их, чтобы побеждать врагов и боссов."]
                                  ):
                screen.blit(font_large.render(line, True, YELLOW if i==0 else WHITE), (60, 150 + i*40))
            btn_back = draw_button(screen, "знал", SCREEN_WIDTH//2-100, 600, 200, 50, GRAY, WHITE, mx, my)
            pygame.display.flip(); clock.tick(FPS); continue

        elif game_state == "TELEPORTING":
            screen.fill(BLACK)
            screen.blit(teleport_bg, (0, 0))
            txt = font_title.render("Передача", True, WHITE)
        ...
        if game_state in ["PLAYING", "TRAINING"]:
            ...
            if game_state == "PLAYING":
                ...
                room_info_text, room_info_color = f"on[{current_floor}]floor ", GREEN
                ...
                if is_battle_locked:
                    room_info_text, room_info_color = ("Враг атакует! Уничтожьте их!", ORANGE) if spawn_pending else ("В бою", RED)
                    for s in spawners[:]:
                        s.timer -= 1
                        if s.timer <= 0:
                            if s.e_type == "boss":
                ...
                if portal_spawned: room_info_text, room_info_color = "Враги в этом районе уничтожены. Найдите портал, чтобы перейти на следующий уровень.", PURPLE
                ...
                room_info_text, room_info_color = "-- тренировочный лагерь -- ", ORANGE
            ...
            for enemy, bullet_list in hits.items():
                for b in bullet_list:
                    if getattr(enemy, 'is_shield_soldier', False):
                        effects.add(DamageText(enemy.rect.centerx, enemy.rect.top-10, "иммунитет", custom_color=(150,150,150), is_text=True))
                        continue 
            ...
            if player.invincible_timer <= 0 and player.revive_anim_timer <= 0:
                ...
                if dmg_taken > 0:
                    if player.take_damage(dmg_taken, effects): 
                        ...
                        effects.add(DamageText(player.rect.centerx, player.rect.top - 35, "Воскресение и возрождение", custom_color=(255, 215, 0), is_text=True))
        ...
        if game_state in ["PLAYING", "TRAINING", "SHOP", "TALENT", "PORTAL_CONFIRM", "GAMEOVER_ANIM", "RESULT", "PAUSED"]:
            ...
            if game_mode == "TUTORIAL" and current_room_index >= 0:
                tut_texts = ["Используйте клавиши WASD для перемещения и F для подбора предметов.", 
                             "Для атаки используйте левую кнопку мыши, для зарядки оружия удерживайте кнопку.", 
                             "[ТАБЛАТУРА]: Магазин, 1/2 Смена оружия", 
                             "Космос: Навыки, Использование двух видов оружия одновременно", 
                             "Выберите режим игры, побеждайте врагов и могущественных боссов и выигрывайте." ]
            ...
            for i in range(player.weapon_slots):
                ...
                if i < len(player.weapons):
                    ...
                    if i == player.stolen_weap_idx:
                        txt = font_base.render("-тюлень-", True, RED)
                        ...
                        if slot_width > 50:
                            ...
                            screen.blit(font_base.render(f"вред: {act_dmg}", True, WHITE), (slot_rect.x + 205, slot_rect.y + 10))
            ...
            if game_state == "PAUSED":
                ...
                txt = font_title.render("Пауза игр", True, ORANGE)
                screen.blit(txt, (cx + cw//2 - txt.get_width()//2, cy + 30))
                
                pygame.draw.rect(screen, (50,50,60), (cx + 30, cy + 100, 240, 500), border_radius=10)
                btn_resume  = draw_button(screen, "Продолжить игру", cx + 50, cy + 130, 200, 50, (50,150,50), (100,200,100), mx, my)
                btn_restart = draw_button(screen, "перезапуск", cx + 50, cy + 220, 200, 50, (150,100,50), (200,150,100), mx, my)
                btn_main    = draw_button(screen, "Вернуться в главное меню", cx + 50, cy + 310, 200, 50, GRAY, WHITE, mx, my)
                btn_quit_pause = draw_button(screen, "Выйти из игры", cx + 50, cy + 400, 200, 50, RED, (255,100,100), mx, my) 

                # （TAB）
                tab_stats_hover = draw_button(screen, "Характерные черты", cx + 320, cy + 100, 140, 40, DARK_GRAY if pause_tab != 0 else ORANGE, YELLOW, mx, my)
                tab_weaps_hover = draw_button(screen, "Оружие в рюкзаке", cx + 470, cy + 100, 140, 40, DARK_GRAY if pause_tab != 1 else ORANGE, YELLOW, mx, my)
                tab_talents_hover = draw_button(screen, " талант ", cx + 620, cy + 100, 140, 40, DARK_GRAY if pause_tab != 2 else ORANGE, YELLOW, mx, my)

                pygame.draw.rect(screen, (40,40,50), (cx + 300, cy + 150, 460, 450), border_radius=10)
                
                if pause_tab == 0:
                    stat_title = "-Основные свойства панели   -"
                    screen.blit(font_large.render(stat_title, True, CYAN), (cx + 300 + 460//2 - font_large.size(stat_title)[0]//2, cy + 170))
                    
                    stat_col1 = [
                        f"Текущий этаж： on {current_floor} floor ",
                        f"Держа в руках золотые монеты: {coins} ",
                        f"Здоровье : {int(player.hp)} / {player.max_hp} ",
                        f"Щит: {player.shield:.1f} / {player.max_shield}",
                        f"скорость передвижения: {player.speed:.1f}"
                    ]
                    stat_col2 = [
                        f"Урон на расстоянии:  +{int((player.bonus_ranged_mult-1)*100)}%",
                        f"Урон в ближнем бою:  +{int((player.bonus_melee_mult-1)*100)}%",
                        f"Повреждения от взрыва:  +{int((player.bonus_explosion_mult-1)*100)}%",
                        f"скорость атаки:  -{player.bonus_cd_reduction}%",
                    ]
                    ...
                    for k, weapon in enumerate(player.weapons):
                        ...
                        weap_txt = f"{k+1} столбец : [{weapon['name']}] "
                        w2 = f" вред：{aw}  [скорость атаки：{cd}]"
                    ...
                    tt = "- Приобретенный талант- "
                    screen.blit(font_large.render(tt, True, PURPLE), (cx + 300 + 460//2 - font_large.size(tt)[0]//2, cy + 170))
                    talents_names = [all_talents_dict[t_id] for t_id in acquired_talents] if acquired_talents else ["Талант еще не приобретен"]
            ...
            if game_state == "PORTAL_CONFIRM":
                ...
                txt_main = "OK" if game_mode=="TUTORIAL" else "Хотите перейти на следующий уровень?"
                txt = font_large.render(txt_main, True, WHITE)
                screen.blit(txt, (cx + cw//2 - txt.get_width()//2, cy + 40))
                
                btn_confirm_yes = draw_button(screen, "Следующий слой", cx + 20, cy + 110, 150, 50, (50,150,50), (100,200,100), mx, my)
                btn_confirm_no  = draw_button(screen, "Подождите минутк", cx + 230, cy + 110, 150, 50, (150,50,50), (200,100,100), mx, my)
                ...
                screen.blit(font_large.render(f"золото: {coins}", True, CYAN), (px+30, py+80))
                ...
                for i in range(start_idx, end_idx):
                    ...
                    desc_text = f"■ {item['name']} " + ("успех" if item['level'] >= item['max'] else f"Lv ：.{item['level']})")
                ...
                prev_hover = draw_button(screen, "Предыдущая страница", px + 40, py + ph - 70, 160, 40, GRAY, WHITE, mx, my) if shop_page > 0 else False
                next_hover = draw_button(screen, "Следующая страница", px + pw - 250, py + ph - 70, 200, 40, GRAY, WHITE, mx, my) if end_idx < len(shop_items) else False
                screen.blit(font_base.render("Клавиша TAB для выключения：)", True, GRAY), (px+145, py+ph-30))

            elif game_state == "TALENT": 
                overlay = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT)); overlay.set_alpha(230); overlay.fill(BLACK); screen.blit(overlay, (0, 0))
                tal_tt = "Выберите один из трех талантов :"
                ...
                res_txt = "Таможенный досмотр пройден успешно " if is_victory else "Ты мертв"
                ...
                r2 = f"Сложность:{game_mode} ,on【 {current_floor} 】floor"
                screen.blit(font_large.render(r2, True, WHITE), (SCREEN_WIDTH//2 - font_large.size(r2)[0]//2, 180))
                r3 = f"Общее количество полученных золотых монет: 【{total_coins} 】| Получите золотые монеты прямо сейчас： {coins}."
                ...
                for idx, w in enumerate(player.weapons):
                    ...
                    screen.blit(font_base.render(f"Значения панели {act_dmg}", True, (255,100,100)), (px_x-50, 445))
                
                btn_restart_res = draw_button(screen, "Вернуться в главное меню", SCREEN_WIDTH//2-250, 550, 500, 50, GRAY, WHITE, mx, my)
                btn_quit_res = draw_button(screen, "Выйти из игры", SCREEN_WIDTH//2-300, 620, 600, 50, RED, (255,100,100), mx, my)
